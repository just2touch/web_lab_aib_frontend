...
class HeaderBlock(BaseXlsBlock):
    ...
    def write_block_data(self):
        ...
        self.worksheet.write(self.row, self.column, date_now, tf)
        ...
        self.worksheet.write(self.row, self.column, period, tf)

        self.column = 0
        self.row += 2


class ClientsBlock(BaseXlsBlock):
    ...
    def write_block_header(self):
        ...
        t2f = self.workbook.add_format(title_2_format)
        t2f.set_text_wrap()
        t2f.set_align('center')

        self.row = 5
        self.column = 0
        self.worksheet.write(self.row, self.column, self.NAME, hf)
    ...
    def write_block_data(self):
        ...
        for q in quarters:
            self.worksheet.write(self.row, self.column, q, t2f)
            srt = sorted(quarters[q], key=lambda x: x['payment_amount'])[:10]
            for s in srt:
                self.row += 1
                self.worksheet.write(self.row, self.column, s['fio'])
            self.row -= 10
            self.column += 1
...
class CitiesBlock(BaseXlsBlock):
    ...
    def write_block_data(self):
        ...
        for i, (city, count) in enumerate(top_cities, start=1):
            self.worksheet.write(self.row + i - 1, self.column, f"{i}. {city}")
            self.worksheet.write(self.row + i - 1, self.column + 1, f"{count}")


class BankBlock(BaseXlsBlock):
    ...
    def write_block_data(self):
        ...
        for i, (client_id, balance) in enumerate(top_balances, start=1):
            ...
            self.worksheet.write(self.row + i - 1, self.column, f"{i}. {fio}")
            self.worksheet.write(self.row + i - 1, self.column + 1, f"{rounded_balance}")
